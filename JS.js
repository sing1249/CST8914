...
class MenuButtonActions {
  ...
  setFocusToMenuitem(newMenuitem) {
    this.menuitemNodes.forEach((item) => {
      if (item === newMenuitem) {
        item.tabIndex = 0; // Changes the tab index of the current item to 0.
        item.focus(); //This will focus the item
      } else {
        item.tabIndex = -1; // The item that is not selected will have tabindex of -1 and will not be focused.
      }
    });
  }
